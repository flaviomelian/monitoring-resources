...
export default function HistoryChart({
  ...
  const chartData = metrics.map((m) => ({
    ...m,
    ramUsagePercentage:
      m.ramTotalGB > 0 ? Math.round((m.ramUsedGB / m.ramTotalGB) * 100) : 0,
  ...
  return (
    <div
      className={`bg-slate-900 border border-slate-800 ${compact ? "p-2" : "p-6"} rounded-xl flex flex-col h-full`}
    >
      {!compact && (
      ...
      {/* Contenedor del gráfico adaptativo */}
      <div className="w-full h-full text-xs flex-1">
